...
import (
	"image/color"
	"log"
//        "github.com/davecgh/go-spew/spew" // spew.Dump

	"engo.io/ecs"
	"engo.io/engo"
	"engo.io/engo/common"
        "github.com/EngoEngine/revgame/systems"
)
...
var (
	...
	leftButton  = "left"
	rightButton = "right"
	model       = "textures/motw.png"
        cityPointer = "textures/city.png"
	width       = 52
	height      = 73
	levelWidth  float32
	levelHeight float32
...
func (*DefaultScene) Preload() {
	...
	// Load TileMap
	if err := engo.Files.Load("textures/example.tmx"); err != nil {
		panic(err)
	}

        if err := engo.Files.Load("textures/city.png"); err != nil {
		panic(err)
	}
	...
	engo.Input.RegisterButton(downButton, engo.S, engo.ArrowDown)
        engo.Input.RegisterButton("AddCity", engo.Input.Mouse.Button)
}
...
func (scene *DefaultScene) Setup(w *ecs.World) {
	...
	// Setup TileMap
	resource, err := engo.Files.Resource("textures/example.tmx")
	if err != nil {
		panic(err)
	}
